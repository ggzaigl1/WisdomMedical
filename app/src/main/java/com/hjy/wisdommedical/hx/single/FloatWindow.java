...
import com.hjy.wisdommedical.hx.widget.VMDimen;
...
public class FloatWindow {
        ...
        floatView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    Log.d("start x: %f, y: %f", startX + "--" + startY);
                    break;
                case MotionEvent.ACTION_MOVE:
                    Log.d("move x: %f, y: %f", event.getRawX() + "--" + event.getRawY());
    ...
    private void refreshCallView(CallEvent event) {
        ...
        case CONNECTING: // 正在呼叫对方，TODO 没见回调过
            Log.i("FloatWindow", "正在呼叫对方" + callError);
            break;
        case CONNECTED: // 正在等待对方接受呼叫申请（对方申请与你进行通话）
            Log.i("FloatWindow", "正在连接" + callError);
            break;
        case ACCEPTED: // 通话已接通
            Log.i("FloatWindow", "通话已接通");
            break;
        case DISCONNECTED: // 通话已中断
            Log.i("FloatWindow", "通话已结束" + callError);
        ...
        case NETWORK_UNSTABLE:
            if (callError == EMCallStateChangeListener.CallError.ERROR_NO_DATA) {
                Log.i("FloatWindow", "没有通话数据" + callError);
            } else {
                Log.i("FloatWindow", "网络不稳定" + callError);
            }
            break;
        case NETWORK_NORMAL:
            Log.i("FloatWindow", "网络正常");
            break;
        case VIDEO_PAUSE:
            Log.i("FloatWindow", "视频传输已暂停");
            break;
        case VIDEO_RESUME:
            Log.i("FloatWindow", "视频传输已恢复");
            break;
        case VOICE_PAUSE:
            Log.i("FloatWindow", "语音传输已暂停");
            break;
        case VOICE_RESUME:
            Log.i("FloatWindow", "语音传输已恢复");
            break;
        }
    }
