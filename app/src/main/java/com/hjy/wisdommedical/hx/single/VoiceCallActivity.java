...
import com.hjy.wisdommedical.util.SelectorUtils;
...
public class VoiceCallActivity extends CallActivity {
    ...
    protected int getContentView() {
        return R.layout.hx_call_voice_act;
    }
    ...
    protected void initView() {
        ...
        Drawable recording = VectorDrawableCompat.create(mContext.getResources(), R.drawable.svg_hx_recording, mContext.getTheme());
        ...
        Drawable handsFree = VectorDrawableCompat.create(mContext.getResources(), R.drawable.svg_hx_hands_free, mContext.getTheme());
     ...
     * 界面控件点击监听器
     */
    @OnClick({R.id.btn_exit_full_screen, R.id.btn_mic_switch, R.id.btn_speaker_switch, R.id.fab_reject_call,
    ...
    private void refreshCallView(CallEvent event) {
        ...
        switch (callState) {
            case CONNECTING: // 正在呼叫对方，TODO 没见回调过
                Log.i("voice", "正在呼叫对方" + callError);
                break;
            case CONNECTED: // 正在等待对方接受呼叫申请（对方申请与你进行通话）
                Log.i("voice", "正在连接" + callError);
