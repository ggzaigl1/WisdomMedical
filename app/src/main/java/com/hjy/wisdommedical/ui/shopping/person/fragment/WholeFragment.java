            ...
            switch (view.getId()) {
                case R.id.tv_left:
                    if (mAdapter.getData().get(position).getOrderStatus() == 1) {
                        T.showShort("取消订单");
                    } else if (mAdapter.getData().get(position).getOrderStatus() == 3) {
                        JumpUtils.jump(mContext, CheckLogisticsActivity.class, null);
                        //去评价
                    } else if (mAdapter.getData().get(position).getOrderStatus() == 4 || mAdapter.getData().get(position).getOrderStatus() == 6) {
