...
//                lp.width = DensityUtil.getScreenWidth(getContext()) - 2 * Utils.dp2px(getContext(), margin);
                lp.width = ScreenUtils.getScreenWidth() - 2 * DensityUtils.dp2px(MyApplication.getAppContext(),(float) margin);
            } else {
                lp.width = DensityUtil.dp2px(width);
            }
            //设置dialog高度
            if (height == 0) {
                lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
            } else {
                lp.height = DensityUtil.dp2px(height);
            }
