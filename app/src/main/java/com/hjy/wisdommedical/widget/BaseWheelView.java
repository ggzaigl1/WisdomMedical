...
/**
 * Created by 初夏小溪 on 2018/7/5 0005.
 */
...
public class BaseWheelView extends ScrollView {
    ...
    public static final String TAG = BaseWheelView.class.getSimpleName();
...
//    private ScrollView scrollView;
    ...
    public BaseWheelView(Context context, AttributeSet attrs, int defStyle) {
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            BaseWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    BaseWheelView.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            BaseWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    BaseWheelView.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    BaseWheelView.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
    }
    ...
    private TextView createView(String item) {
        ...
        TextPaint paint = tv.getPaint();//加粗
...
//            position = divided + offset + 1;
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (position == i) {
                itemView.setTextColor(Color.parseColor("#6f8bf0"));
            } else {
                itemView.setTextColor(Color.parseColor("#c3c6cb"));
            }
        }
    }
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                BaseWheelView.this.smoothScrollTo(0, p * itemHeight);
            }
        });
